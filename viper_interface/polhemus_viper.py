...
class ECmdActions(Enum):
    cmd_action_set = 0  # Used with a command and payload to change state.
    cmd_action_get = 1  # Used with a command to request current state.
    cmd_action_reset = 2  # Used with a command to restore default state.
    # Command received and processed, possible payload included.
    cmd_action_ack = 3
    # Command received, error found: always includes NAK_INFO struct in payload.
    cmd_action_nak = 4
    # Command received and processed, warning condition: payload includes NAK_INFO struct.
...
class EViperCmds(Enum):
    # cmd  actions		scope	payload struct		persist		cmd type	arg1/scope?		arg2/qualifier?
    # -----------------------------------------------------------------------------------------------------
    # sensor cfg - persistable
    cmd_hemisphere = 0  # 0   set/get/reset,	sensor	HEMISPHERE_CONFIG		p		cfg			sensornum
    cmd_filter = 1  # 1   set/get/reset,	sensor	FILTER_CONFIG			p		cfg			sensornum		eFilterTargets
    cmd_tip_offset = 2  # 2   set/get/reset,	sensor	TIP_OFFSET_CONFIG		 		cfg			sensornum
    cmd_increment = 3  # 3   set/get/reset,	sensor	INCREMENT_CONFIG		p		cfg			sensornum
    # sensor cfg - runtime
    # 4   set/get/reset,	sensor	BORESIGHT_CONFIG				cfg			sensornum		bool reset Origin
    cmd_boresight = 4
# sensor cmds - read-only
    cmd_sensor_whoami = 5  # 5      /get/     ,	sensor	WHOAMI_STRUCT					read-only
# seu cfg - persistable
    cmd_framerate = 6  # 6   set/get/reset,	SEU		FRAMERATE_CONFIG(ENUM)	p		cfg
    cmd_units = 7  # 7   set/get/reset,	SEU		UNITS_CONFIG			p		cfg
    cmd_src_rotation = 8  # 8   set/get/reset,	SEU		SRCROT_CONFIG			p		cfg			sourcenum
    cmd_sync_mode = 9  # 9   set/get/reset,	SEU		BINARY_CONFIG			p		cfg-binary
    cmd_station_map = 10  # 10     /get/     ,	SEU		STATION_MAP				p		read-only
    cmd_stylus = 11  # 11  set/get/reset,	SEU		STYLUS_CONFIG(ENUM)		p		cfg
    cmd_seuid = 12  # 12  set/get/reset,	SEU		VIPER_SEUID				p		cfg
    cmd_dual_output = 13  # 13  set/get/reset,	SEU		BINARY_CONFIG			p		cfg-binary
    cmd_serial_config = 14  # 14  set/get/reset,	SEU		SERIAL_CONFIG			p		cfg
    cmd_block_config = 15  # 15         /reset,	SEU		none					p		cfg
# seu cfg - runtime
    cmd_frame_count = 16  # not implemented 16     /   /reset,	SEU		none							executive
    cmd_bit = 17  # 17     /get/     ,	SEU		BIT_STRUCT						executive
# seu cmds - pno
    cmd_single_pno = 18  # 18  get/   /     ,	SEU		SEUPNO+SENFRAMEDATA				pno							ePnoMode
    cmd_continuous_pno = 19  # 19  set/   /reset,	SEU		CONT_FC_CONFIG					pno							ePnoMode
# seu cmds - read-only
    cmd_whoami = 20  # 20     /get/     ,	SEU		WHOAMI_STRUCT					read-only
# executive cmds
    cmd_initialize = 21  # 21  set/   /     ,	SEU		none							executive
    cmd_persist = 22  # 22  set/   /     ,	SEU		none							executive
    cmd_enable_map = 23  # 23  set/   /reset,	SEU		none							executive

    # 24  set/get/reset,	sensor	FTTMODE_CONFIG(ENUM)	p		cfg			sensornum
    cmd_ftt_mode = 24
    cmd_map_status = 25  # 25     /get/     ,	SEU
    # not implemented 26         /reset,	sensor	none					p		cfg			sensornum
    cmd_sensor_blockcfg = 26
    cmd_source_cfg = 27  # 27  set/get/reset,	SEU		SRC_CONFIG				p		cfg			sourcenum
    cmd_predfilter_cfg = 28  # 28  set/get/reset,	sensor	PF_CONFIG				p		cfg			sensornum
    cmd_predfilter_ext = 29  # 29  set/get/reset,	sensor	PF_CONFIG_EXT			p		cfg			sensornum
    cmd_src_select = 30  # 30  set/get/reset,	sensor	SRC_SEL_CONFIG			p		cfg			sensornum
    # 31  set/get/reset,	sensor	SNS_ORIG_CONFIG(ENUM)	p		cfg			sensornum
    cmd_sns_origin = 31
    cmd_sns_virtual = 32  # 32  set/get/reset,	sensor	VIRTUAL_SNS_CONFIG		p		cfg			sensornum
    cmd_src_whoami = 33  # 33     /get/     ,	source	WHOAMI_STRUCT    		p		read-only	sourcenum
    cmd_max = 34


# ==============================================================================

class EViperOriUnits(Enum):
    """
    ...
    ori_euler_radian = 1  # Euler angles in radians
    ori_quaternion = 2   # Quaternions
    ori_max = 3


class EViperPosUnits(Enum):
    """
    ...
    pos_inch = 0  # Inches
    pos_foot = 1  # Feet
    pos_cm = 2   # Centimeters
    pos_meter = 3  # Meters
    pos_max = 4


class EViperFrameRate(Enum):
    """
...
class EViperFilterTargets(Enum):
    """
    ...
    filter_trgt_ori = 1  # Target orientation
    filter_trgt_pos = 2  # Target position
    filter_trgt_max = 3


class EViperFilterLevel(Enum):
    """
    ...
    """
    filter_lvl_none = 0  # 0000 no filtering
    filter_lvl_light = 1		# 0001 traditional light filtering
    filter_lvl_medium = 2		# 0002  traditional medium filtering
    filter_lvl_heavy = 3		# 0003  traditional heavy filtering
    filter_lvl_custom = 4		# 0004  traditional custom filtering
    filter_lvl_e_light = 5  # 0005  enhanced light filtering
    filter_lvl_e_medium = 6  # 0006  enhanced medium filtering
    filter_lvl_e_heavy = 7 	# 0007  enhanced heavy filtering
    filter_lvl_reserved = 8  # 0008  reserved

    filter_lvl_max = 9


class EStylusMode(Enum):
    """
    Valid values for cmd_stylusmode in stylus_config
    """
    stylus_mode_mark = 0
    stylus_mode_point = 1
    stylus_mode_line = 2
    stylus_mode_toggle = 3
    stylus_mode_max = 4
...
class EFTTMode(Enum):
    ...
    """
    ftt_mode_none = 0
    ftt_mode_1 = 1  # aka stationary source mode
    ftt_mode_2 = 2  # aka moving source mode
    ftt_mode_3 = 3
    ftt_mode_max = 4


class ESensorOrigin(Enum):
    """
    Sensor Origin values used for @ref CMD_SNS_ORIGIN
    """
    sns_orig_src1 = 0		# Default.  Sensor Origin is local source
    sns_orig_src2 = 1		# Sensor Origin is Source 2
    sns_orig_src3 = 2		# Sensor Origin is Source 3
    sns_orig_src4 = 3		# Sensor Origin is Source 4
    # Sensor origin is common to the device, usually implied by source configuration
    sns_orig_common = 4
    sns_orig_max = 5
...
class ePnoMode(Enum):
    ...
    # position and orientation plus acceleration using @ref PNO_A struct per sensor
    e_pnomode_a = 1
    e_pnomode_max = 2


class EViperHemisphere(Enum):
    """
    Hemisphere values used for @ref CMD_SOURCE_CFG
    """
    e_vp_hem_pos_x = 0  # +X Hemisphere
    e_vp_hem_pos_y = 1  # +Y Hemisphere
    e_vp_hem_pos_z = 2  # +Z Hemisphere
    e_vp_hem_neg_x = 3  # -X Hemisphere
    e_vp_hem_neg_y = 4  # -Y Hemisphere
    e_vp_hem_neg_z = 5  # -Z Hemisphere
    e_vp_hem_auto = 6  # Auto-Hemisphere
    e_vp_hem_max = 7
...
class EBaud(Enum):
    ...
    e_br_2400 = 1       # not valid for viper
    e_br_4800 = 2		# not valid for viper
    e_br_9600 = 3		# not valid for viper
    e_br_19200 = 4		# not valid for viper
    e_br_38400 = 5		# 38,400 baud
    e_br_57600 = 6		# 57,600 baud
    e_br_115200 = 7		# 115,200 baud (default)
    e_br_230400 = 8     # 230,400 baud
    e_br_460800 = 9    # 460,800 baud
    e_br_921600 = 10    # 921,600 baud
    e_br_1843200 = 11   # 1,843,200 baud
    e_br_3686400 = 12   # 3,686,400 baud
    e_br_7372800 = 13   # 7,372,800 baud
    e_br_max = 14
...
class EParity(Enum):
    ...
    """
    e_parity_none = 0       # no parity
    e_parity_odd = 1        # odd parity
    e_parity_even = 2       # even parity
    e_parity_max = 3
...
class EBITCode(Enum):
    ...
    """
    e_biterr_none = 0       # no error.  self-test and startup successful.
    e_biterr_sensor = 1     # sensor error
    e_biterr_sensor_port = 2        # sensor port error
    # source error (could also indicate a source port error)
    e_biterr_source = 3
    e_biterr_seu = 4  # seu error
    e_biterr_dupfreq = 5        # duplicate source frequencies detected.
    e_biterr_reserved1 = 6  # reserved.
    e_biterr_reserved2 = 7  # reserved.
    e_biterr_max = 8


class EViperFloorLevels(Enum):
    """
    ...
    """
    floor_lvl_none = 0  # no floor compensation
    floor_lvl_50 = 1  # source 50cm off floor
    floor_lvl_75 = 2  # source 75cm off floor
    floor_lvl_100 = 3  # source 100cm off floor
    floor_lvl_max = 4


# Command frame preamble. All command frames begin with 'V''P''R''C'.
VIPER_CMD_PREAMBLE = b'0x43525056'

# Position and orientation (PNO) frame preamble. All PNO frames begin with 'V''P''R''P'.
VIPER_PNO_PREAMBLE = b'0x50525056'

# The first 64 bytes of all frames to/from the device.
...
@dataclass
class VPFrameHeader:
    # 4 byte preamble @ref VIPER_CMD_PREAMBLE or VIPER_PNO_PREAMBLE indicates frame type.
    preamble: np.uint32
    # 32-bit frame size indicates the number of bytes to follow, including terminating CRC.
    size: np.uint32

# All frames ae terminated by a 32-bit calculated CRC value.
...
@dataclass
class HPINFO:
    bfPnoMod: np.uint32 = 4
    bfBITerr: np.uint32 = 4
    bfReserved1: np.uint32 = 8
    bfReserved2: np.uint32 = 16
...
class SFINFO:
    bfSnum: np.uint32 = 7
    bfSvirt: np.uint32 = 1
    bfPosUnits: np.uint32 = 2
    bfOriUnits: np.uint32 = 2
    bfBtnState0: np.uint32 = 1
    bfBtnState1: np.uint32 = 1
    bfDistortion: np.uint32 = 8
    bfAuxInput: np.uint32 = 10
...
@dataclass
class SENFRAMEDATE_A:
    SFinfo: SFINFO  # 4 bytes
    pno: PNO_ACC  # 28 bytes
...
NAME_SIZE = 64  # Number of WhoAmI device_name ASCII characters: 64
SERNUM_SIZE = 32  # Number of WhoAmI device_serial_number ASCII characters: 32
PN_SIZE = 32  # Number of WhoAmI part number ASCII characters: 32
...
# 		struct _bf {				//!< bitfield representation of RS422 settings
